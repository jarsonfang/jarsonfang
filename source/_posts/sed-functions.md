...
*   说明 : 用函数参数 = 来印出资料的行数。
*   sed 命令列如下 :
 [cc lang="bash"]sed -e '=' input.dat[/cc]
执行上述命令後 , 输出的结果如下 :
> 1> 
> The UNIX> 
> 2> 
...
*   sed 命令列 :
[cc lang="bash"]sed -n -e 'N' -e 'P' input.dat[/cc]
执行上述命令後 , 输出的结果如下 :
> The> 
> System
...
<table border=1>
<tr><th>命令</th><th>缩写</th><th>功能</th></tr>
<tr><td>Hold</td><td>h 或 H</td><td>将模式空间的内容复制或追加到保持空间</td></tr>
<tr><td>Get</td><td>g 或 G</td><td>将保持空间的内容复制或追加到模式空间</td></tr>
<tr><td>Exchang</td><td>x</td><td>交换模式空间和保持空间的内容</td></tr>
</table>
...
[cc lang="bash"]sed -e '1h' -e '3x' input.dat[/cc]
